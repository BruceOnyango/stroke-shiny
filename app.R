...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      body { font-family: 'Segoe UI', sans-serif; background-color: #f4f7f7; }
      .app-header { background:#1a3a4a; color:white; padding:18px 20px;
                    margin:-15px -15px 20px -15px; border-radius:0; }
      .app-header h2 { margin:0; font-size:1.6em; }
      .app-header p  { margin:4px 0 0 0; font-size:0.85em; opacity:0.75; }
      .nav-tabs > li > a { color:#1a3a4a; font-weight:bold; }
      .nav-tabs > li.active > a { color:white !important;
        background-color:#2a9d8f !important; border-color:#2a9d8f !important; }
      .section-header { background:#264653; color:white; padding:10px 15px;
        border-radius:5px; margin:15px 0 10px 0; font-size:1em; }
      .card { background:white; border-radius:8px; padding:18px;
              margin-bottom:15px; box-shadow:0 2px 5px rgba(0,0,0,0.08); }
      .impact-box { background:#1a3a4a; color:white; border-radius:8px;
        padding:18px 12px; margin:6px; text-align:center;
        box-shadow:0 3px 8px rgba(0,0,0,0.15); }
      .impact-value { font-size:2.2em; font-weight:bold; color:#4db6ac; }
      .impact-label { font-size:0.82em; margin-top:6px; opacity:0.85; line-height:1.3; }
      .metric-box { background:white; border-radius:8px; padding:15px; margin:6px;
        text-align:center; box-shadow:0 2px 5px rgba(0,0,0,0.08);
        border-top:4px solid #2a9d8f; }
      .metric-value { font-size:1.9em; font-weight:bold; color:#2a9d8f; }
      .metric-label { font-size:0.82em; color:#555; margin-top:5px; }
      .strategic-card { background:white; border-radius:8px; padding:18px;
        margin:6px; box-shadow:0 2px 6px rgba(0,0,0,0.08); }
      .card-high { border-top:5px solid #e76f51; }
      .card-med  { border-top:5px solid #e9c46a; }
      .card-low  { border-top:5px solid #2a9d8f; }
      .insight-box { background:#e8f4f3; border-left:4px solid #2a9d8f;
        padding:10px 15px; border-radius:4px; margin-bottom:15px; font-size:13px; }
      .predict-btn { background-color:#2a9d8f; color:white; border:none;
        padding:13px 0; width:100%; font-size:1em; font-weight:bold;
        border-radius:6px; cursor:pointer; margin-top:12px; letter-spacing:0.5px; }
      .predict-btn:hover { background-color:#264653; color:white; }
      .result-card { background:white; border-radius:8px; padding:22px;
  ...
  tabsetPanel(
    ...
    tabPanel("Executive Dashboard",
      br(),
      h4(class="section-header", "Why Early Stroke Detection Matters"),
      fluidRow(
        column(3, div(class="impact-box",
          div(class="impact-value", "7.2x"),
          div(class="impact-label", "Higher stroke risk in patients aged 60+ vs under 60")
        )),
        column(3, div(class="impact-box",
          div(class="impact-value", "3.3x"),
          div(class="impact-label", "Higher stroke risk in hypertensive patients")
        )),
        column(3, div(class="impact-box",
          div(class="impact-value", "93%"),
          div(class="impact-label", "Of real stroke cases correctly identified by this model")
        )),
        column(3, div(class="impact-box",
          div(class="impact-value", "99.4%"),
          div(class="impact-label", "Negative predictive value — cleared patients are safe")
        ))
      ),
      br(),
      h4(class="section-header", "The Evidence — Stroke Risk Across Patient Groups"),
      fluidRow(
        column(4,
          h5("Stroke Rate by Age Group",
             style="text-align:center; font-weight:bold; color:#1a3a4a;"),
          div(class="card",
            withSpinner(plotOutput("exec_age_plot", height="260px"), type=6, color="#2a9d8f")
          )
        ),
        column(4,
          h5("Stroke Rate by Hypertension Status",
             style="text-align:center; font-weight:bold; color:#1a3a4a;"),
          div(class="card",
            withSpinner(plotOutput("exec_hypert_plot", height="260px"), type=6, color="#2a9d8f")
          )
        ),
        column(4,
          h5("Stroke Rate by Risk Group",
             style="text-align:center; font-weight:bold; color:#1a3a4a;"),
          div(class="card",
            withSpinner(plotOutput("exec_riskrate_plot", height="260px"), type=6, color="#2a9d8f")
          )
        )
      ),
      br(),
      ...
      fluidRow(
        column(4,
          div(class="strategic-card card-high",
            h5("High Risk Protocol", style="color:#e76f51; margin-top:0; font-size:1.05em;"),
            p(strong("Who:"), " Patients aged 60+ with hypertension"),
            p(strong("Stroke Rate:"), " ~13%"),
            p(strong("Action:"), " Immediate specialist referral at every consultation"),
            p(strong("Business Case:"), " Preventing one stroke avoids cost of long-term
               rehabilitation, extended inpatient stay, and ongoing specialist care")
          )
        ),
        column(4,
          div(class="strategic-card card-med",
            h5("Medium Risk Protocol", style="color:#c49a1a; margin-top:0; font-size:1.05em;"),
            p(strong("Who:"), " Aged 60+ OR hypertension OR heart disease"),
            p(strong("Stroke Rate:"), " ~8-10%"),
            p(strong("Action:"), " Quarterly monitoring, lifestyle intervention programme"),
            p(strong("Business Case:"), " Early lifestyle intervention costs significantly less
               than acute stroke treatment and reduces readmission risk")
          )
        ),
        column(4,
          div(class="strategic-card card-low",
            h5("Low Risk Protocol", style="color:#2a9d8f; margin-top:0; font-size:1.05em;"),
            p(strong("Who:"), " Age under 60, no hypertension, no heart disease"),
            p(strong("Stroke Rate:"), " ~1.8%"),
            p(strong("Action:"), " Annual standard screening, smoking history recorded"),
            p(strong("Business Case:"), " Efficient resource allocation — clinical staff time
      ...
      fluidRow(
        column(6,
          div(class="card",
            withSpinner(plotOutput("risk_group_plot", height="300px"), type=6, color="#2a9d8f")
          )
        ),
        column(6,
          div(class="card",
            withSpinner(plotOutput("scatter_plot", height="300px"), type=6, color="#2a9d8f")
          )
        )
      )
    ),
    ...
    tabPanel("Clinical Decision Support",
      ...
      fluidRow(
        column(6,
          ...
          div(class="card",
            withSpinner(plotOutput("roc_plot", height="350px"), type=6, color="#2a9d8f")
          )
        ),
        column(6,
          h4(class="section-header", "Model Comparison"),
          div(class="card",
            withSpinner(plotOutput("model_compare_plot", height="350px"), type=6, color="#2a9d8f")
          )
        )
      ),
      br(),
      fluidRow(
        column(6,
          ...
          div(class="card",
            withSpinner(plotOutput("importance_plot", height="320px"), type=6, color="#2a9d8f")
          )
        ),
        column(6,
          h4(class="section-header", "Threshold Tuning Effect"),
          div(class="card",
            withSpinner(plotOutput("threshold_plot", height="320px"), type=6, color="#2a9d8f")
          )
        )
      ),
      br(),
    ...
    tabPanel("Data Overview",
      ...
      fluidRow(
        column(4,
          div(class="card",
            withSpinner(plotOutput("class_plot", height="280px"), type=6, color="#2a9d8f")
          )
        ),
        column(8,
          div(class="card",
            withSpinner(plotOutput("means_plot", height="280px"), type=6, color="#2a9d8f")
          )
        )
      ),
      br(),
      fluidRow(
        column(12,
          div(class="card",
            ...
            withSpinner(plotOutput("hist_plot", height="260px"), type=6, color="#2a9d8f")
          )
        )
      )
    ),
    ...
    tabPanel("Risk Factors",
      ...
      fluidRow(
        column(7,
          div(class="card",
            ...
            withSpinner(plotOutput("scatter_rf", height="320px"), type=6, color="#2a9d8f")
          )
        ),
        column(5,
          div(class="card",
            withSpinner(plotOutput("importance_plot2", height="380px"), type=6, color="#2a9d8f")
          )
        )
      ),
      br(),
      fluidRow(
        column(12,
          div(class="card",
            withSpinner(plotOutput("age_density_plot", height="280px"), type=6, color="#2a9d8f")
          )
        )
      )
    ),
    ...
    tabPanel("My Risk Assessment",
      ...
      fluidRow(
        column(4,
          wellPanel(
            h4("Enter Your Details", style="color:#1a3a4a; margin-top:0;"),
            sliderInput("age_in", "Age (years)", min=1, max=82, value=45),
            sliderInput("glucose_in", "Average Glucose (mg/dL)", min=55, max=272, value=100),
            sliderInput("bmi_in", "BMI (kg/m²)", min=10, max=98, value=28),
            selectInput("hypertension_in", "Do you have hypertension?",
                        choices=c("No"=0, "Yes"=1)),
            selectInput("heart_disease_in", "Do you have heart disease?",
                        choices=c("No"=0, "Yes"=1)),
            selectInput("gender_in", "Gender", choices=c("Female"=0, "Male"=1)),
            selectInput("ever_married_in", "Ever Married?", choices=c("No"=0, "Yes"=1)),
            selectInput("residence_in", "Where do you live?",
                        choices=c("Rural"=0, "Urban"=1)),
            selectInput("work_type_in", "Work Type",
                        choices=c("Private","Self-employed","Govt_job",
                                  "children","Never_worked")),
            selectInput("smoking_in", "Smoking Status",
                        choices=c("never smoked","formerly smoked","smokes","Unknown")),
            tags$button("Calculate My Risk", id="predict",
                        class="predict-btn action-button", type="button")
          )
        ),
        column(8,
...
server <- function(input, output){
  ...
  prediction <- eventReactive(input$predict, {
    new_patient <- data.frame(
      ...
      smoking_statusformerly.smoked = as.integer(input$smoking_in=="formerly smoked"),
      smoking_statusnever.smoked    = as.integer(input$smoking_in=="never smoked"),
      smoking_statussmokes          = as.integer(input$smoking_in=="smokes"),
      smoking_statusUnknown         = as.integer(input$smoking_in=="Unknown"),
      glucose_outlier_flag = as.integer(as.numeric(input$glucose_in) > 145),
      glucose_bmi  = as.numeric(input$glucose_in) * as.numeric(input$bmi_in),
      age_glucose  = as.numeric(input$age_in) * as.numeric(input$glucose_in),
      age_bmi      = as.numeric(input$age_in) * as.numeric(input$bmi_in),
      hypert_age   = as.numeric(input$hypertension_in) * as.numeric(input$age_in)
    )
  ...
  output$patient_result <- renderUI({
    req(prediction())
    p   <- prediction()
    col <- ifelse(p$outcome=="HIGH RISK", "#e76f51", "#2a9d8f")
    ...
    if(p$outcome=="HIGH RISK"){
      div(
        p("Your results suggest you may be at elevated risk of stroke.
           This does not mean you will have a stroke — it means you should
           speak to your doctor as soon as possible for a full clinical assessment."),
        p("Early detection and lifestyle changes can significantly reduce your risk.")
      )
    } else {
      div(
        p("Your results suggest you are currently at lower risk of stroke.
           Continue maintaining a healthy lifestyle and attend your regular annual check-ups."),
  ...
  output$exec_age_plot <- renderPlot({
    ...
    age_data$Group <- factor(age_data$Group, levels=c("Under 40","40-59","60+"))
    ggplot(age_data, aes(x=Group, y=Rate, fill=Group)) +
      geom_bar(stat="identity", width=0.55) +
      geom_text(aes(label=paste0(Rate,"%")), vjust=-0.5, fontface="bold", size=4.5) +
      scale_fill_manual(values=c("Under 40"="#4db6ac","40-59"="#2a9d8f","60+"="#1a3a4a")) +
  ...
  output$exec_hypert_plot <- renderPlot({
    ...
    ggplot(hyp_data, aes(x=Group, y=Rate, fill=Group)) +
      ...
      geom_text(aes(label=paste0(Rate,"%")), vjust=-0.5, fontface="bold", size=4.5) +
      scale_fill_manual(values=c("No Hypertension"="#4db6ac","Hypertension"="#1a3a4a")) +
  ...
  output$exec_riskrate_plot <- renderPlot({
    rg_rate <- df %>%
      group_by(risk_group) %>%
      summarise(stroke_rate=round(mean(stroke)*100, 1), .groups="drop")
    ggplot(rg_rate, aes(x=reorder(risk_group,-stroke_rate),
                         y=stroke_rate, fill=risk_group)) +
      geom_bar(stat="identity", width=0.45) +
      geom_text(aes(label=paste0(stroke_rate,"%")), vjust=-0.5, fontface="bold", size=4.5) +
  ...
  output$roc_plot <- renderPlot({
    ...
    model_cols  <- c("#1a3a4a","#2a9d8f","#4db6ac")
    ...
    ggplot(roc_df, aes(x=FPR, y=TPR, colour=Model)) +
      ...
      labs(title="ROC Curves — Model Comparison",
           ...
           y="True Positive Rate (Sensitivity)", colour="") +
      theme_app()
  })
  ...
  output$model_compare_plot <- renderPlot({
    metrics <- data.frame(
      ...
      Metric = rep(c("AUC","Sensitivity","Specificity","Accuracy"), each=3),
      Value  = c(0.827, 0.753, 0.806,
                 0.932, 0.878, 0.905,
                 0.588, 0.627, 0.587,
                 0.604, 0.639, 0.603)
    )
    ...
    ggplot(metrics, aes(x=Model, y=Value, fill=Model)) +
      ...
      scale_fill_manual(values=c(
        "Logistic Regression" = "#1a3a4a",
        "Decision Tree"       = "#2a9d8f",
  ...
  output$importance_plot <- renderPlot({
    imp_data <- data.frame(
      Feature=c("age","age_glucose","age_bmi",
                ...
                "hypertension","gender","avg_glucose_level","glucose_bmi"),
      Score=c(100,78.21,58.99,41.45,40.37,
              36.91,33.68,32.44,30.27,
              30.13,29.64,28.81,25.97,24.76,23.41)
    )
    imp_data$color_group <- ifelse(imp_data$Score > 50, "high", "low")
    ggplot(imp_data, aes(x=reorder(Feature,Score), y=Score, fill=color_group)) +
      geom_bar(stat="identity") +
      geom_text(aes(label=round(Score,1)), hjust=-0.1, size=3.2) +
      coord_flip() +
      scale_fill_manual(values=c("high"="#1a3a4a","low"="#2a9d8f")) +
  ...
  output$threshold_plot <- renderPlot({
    ...
    thresh_data$Threshold <- factor(thresh_data$Threshold,
                                    levels=c("Default (0.50)","Tuned (0.30)"))
    ggplot(thresh_data, aes(x=Metric, y=Value, fill=Threshold)) +
      ...
      scale_fill_manual(values=c("Default (0.50)"="#4db6ac","Tuned (0.30)"="#1a3a4a")) +
      scale_y_continuous(limits=c(0,1.15), expand=expansion(mult=c(0,0.1))) +
  ...
  output$means_plot <- renderPlot({
    means <- df %>%
      group_by(stroke_f) %>%
      summarise(Age=mean(age), Glucose=mean(avg_glucose_level),
                BMI=mean(bmi), .groups="drop") %>%
      melt(id.vars="stroke_f", variable.name="Feature", value.name="Mean")
  ...
  output$scatter_rf <- renderPlot({
    ggplot(df, aes_string(x=input$x_axis, y=input$y_axis, colour="stroke_f")) +
  ...
  output$importance_plot2 <- renderPlot({
    ...
    ggplot(imp_data, aes(x=reorder(Feature,Score), y=Score, fill=color_group)) +
      ...
      scale_fill_manual(values=c("high"="#1a3a4a","low"="#2a9d8f")) +
      scale_y_continuous(expand=expansion(mult=c(0,0.18))) +
      labs(title="Feature Importance", subtitle="Top 15 predictors",
           x="", y="Importance Score") +
      theme_app() + theme(legend.position="none")
  })
  ...
  output$age_density_plot <- renderPlot({
    ggplot(df, aes(x=age, fill=stroke_f)) +
      ...
      scale_fill_manual(values=COLOURS) +
      geom_vline(xintercept=60, linetype="dashed", colour="#333333", linewidth=0.8) +
      annotate("text", x=62, y=0.035, label="Risk accelerates\nafter age 60",
